...
def main(_argv):
    ...
    images_names = os.listdir(FLAGS.input_images)

    for image_name in images_names:
        img_raw = tf.image.decode_image(
            open(os.path.join(FLAGS.input_images, image_name), 'rb').read(), channels=3)
        ...
        img, cropped_image = draw_outputs(img, (boxes, scores, classes, nums), class_names)
        cv2.imwrite(os.path.join(FLAGS.output, image_name.split('.')[0]) + '.jpg', img)
        for i,image in enumerate(cropped_image):
            cv2.imwrite(os.path.join(FLAGS.cropped_output, image_name.split('.')[0]) + '_' + str(i) + '.jpg', image)
        logging.info('output saved to: {}'.format(os.path.join(FLAGS.output, image_name.split('.')[0]) + '.jpg'))
